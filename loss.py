...
def get_loss(args):
    ...
    return criterion, criterion_val, L1_loss
...
class L1Loss(nn.Module):
    ...
    def __call__(self, in0, in1):
        b, c, w, h = in0.size()
        pred1 = in0.view(-1).cuda()
        pred2 = in1.view(-1).cuda()
        # print(pred1)
        # print(pred2)
        output = torch.sum(torch.abs(pred1 - pred2)) / (w * h * c * b)
        return output
        # return torch.sum(torch.abs(in0 - in1), dim=1)
